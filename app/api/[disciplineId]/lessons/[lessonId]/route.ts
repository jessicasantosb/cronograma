...
export async function GET(
  ...
  try {
    ...
    if (error instanceof Error) console.log('[LESSONS_GET]', error.message);
...
export async function PATCH(
  ...
  try {
    ...
    if (!deadline) {
      return new NextResponse('Deadline is required', { status: 400 });
    }
    ...
    if (error instanceof Error) console.log('[LESSONS_PATCH]', error.message);
    return new NextResponse('Internal Server Error', { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Error) console.log('[LESSONS_DELETE]', error.message);
    return new NextResponse('Internal Server Error', { status: 500 });
  }
}
